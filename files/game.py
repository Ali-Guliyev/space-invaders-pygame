...
class Game:
  ...
  def restart(self):
    ...
    playmusic(f"{path}assets/sounds/background-music.mp3")
    self.over_count = 0
...
# Obstacles
# obstacle = Sprite(100, HEIGHT - 170, "assets/images/obstacle.png")
# obstacle.transform(0.3)
  ...
  if game.isActive:
    ...
    for enemy in enemies:
      ...
      if enemy.sprite_rect.right >= WIDTH or enemy.sprite_rect.left <= 0:
        for enemy in enemies:
          ...
          enemy.sprite_rect.y += enemy.down_speed
    ...
    if game.attempts == 0:
      ...
      if (game.over_count == 2):
        ...
        playmusic(f"{path}assets/sounds/lose.mp3");
